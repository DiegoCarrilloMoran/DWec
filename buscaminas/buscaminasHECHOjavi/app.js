document.addEventListener("DOMContentLoaded", () => {
  ...
  const contadorBanderasRestantes =
    document.getElementById("banderas-restantes");
  ...
  function añadeNumeros() {
    for (let i = 0; i < casillas.length; i++) {
      ...
      if (casillas[i].classList.contains("vacio")) {
        ...
        if (i > width && casillas[i - width].classList.contains("bomba"))
          total++;
  ...
  function revelarCasillas(casilla) {
    ...
    setTimeout(() => {
      ...
      if (idCasilla < width * width - 2 && !estaBordeDech)
      ...
      if (idCasilla > width - 1 && !estaBordeDech)
        click(casillas[idCasilla + 1 - width]);

      // Simulamos clik en la casilla anterior de la fila anterior
      if (idCasilla > width + 1 && !estaBordeIzq)
        click(casillas[idCasilla - 1 - width]);
      ...
      if (idCasilla < width * width - width - 2 && !estaBordeDech)
        click(casillas[idCasilla + 1 + width]);

      // Simulamos clik en la casilla anterior de la fila siguiente
      if (idCasilla < width * width - width && !estaBordeIzq)
        click(casillas[idCasilla - 1 + width]);
    }, 10);
  }
  ...
  function crearJuego() {
    ...
    if (numBombas > width * width) {
      alert(
        `El número de bombas no puede ser superior al producto de \"Tamaño\" x \"Tamaño\" (${
          width * width
        })`
      );
      return;
